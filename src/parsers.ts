...
export function parseDeviceInfoBody(body: Uint8Array): HiDockDeviceInfo {
  if (body.length < 4) {
    return {
      version: "unknown",
      serialNumber: "",
  ...
  const serialNumber = trimTrailingNul(
    textDecoder.decode(serialNumberRaw).replace(/\u0000/g, ""),
  ).trim();
...
export function parseDeviceTimeBody(body: Uint8Array): HiDockDeviceTime {
  return {
...
export function parseFileCountBody(body: Uint8Array): number {
  if (body.length === 0) {
    return 0;
  }
  if (body.length !== 4) {
...
function decodeBcdDateTime(value: Uint8Array): string | null {
  if (value.length !== 7) {
    return null;
  }
