...
export class MeetingStorage {
  ...
  async saveMeeting(input: MeetingDocumentInput): Promise<SavedMeetingDocument> {
    ...
    const noteFilePath = await this.uniquePath(
      monthDir,
      `${stamp}-${titleSlug}.md`,
    );
    ...
    await fs.writeFile(
      noteFilePath,
      renderMeetingNote(input),
      "utf8",
    );

    await this.ensureIndexHeader(
      indexPath,
      "# Meeting Index\n\n",
    );
  ...
  getIndexPath(kind: DocumentKind): string {
    return path.join(
      this.rootDir,
      kind === "meeting" ? "meetingindex.md" : "whisperindex.md",
    );
  }
  ...
  private async indexContainsSource(
    indexPath: string,
    sourceFileName: string,
  ): Promise<boolean> {
    try {
...
function renderWhisperNote(input: MeetingDocumentInput): string {
  const brief = truncateWords(input.brief, 14);
  return (
    `# Whisper ${formatTimestampToken(input.timestamp)}\n\n` +
    `- DateTime: ${formatDateTime(input.timestamp)}\n` +
    `- Brief: ${brief}\n` +
    `- Source: ${input.sourceFileName}\n\n` +
    `## Summary\n\n` +
