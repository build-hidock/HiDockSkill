...
const WIKI_EXTRACTION_PROMPT =
  "You are a knowledge extraction assistant. Read the meeting summary below and extract structured information.\n\n" +
...
export function parseExtractionJson(text: string, sourceRef: string): WikiExtractionResult {
  const empty: WikiExtractionResult = { people: [], projects: [], topics: [], decisions: [], actions: [] };
  ...
  try {
    ...
    const people = Array.isArray(obj.people)
      ? (obj.people as Record<string, unknown>[]).map((p) => ({
        name: String(p.name ?? ""),
    ...
    const topics = Array.isArray(obj.topics)
      ? (obj.topics as Record<string, unknown>[]).map((t) => ({
        name: String(t.name ?? ""),
    ...
    const decisions = Array.isArray(obj.decisions)
      ? (obj.decisions as Record<string, unknown>[]).map((d) => ({
    ...
    const actions = Array.isArray(obj.actions)
      ? (obj.actions as Record<string, unknown>[]).map((a) => ({
        ...
        status: (String(a.status ?? "open") === "done" ? "done" : "open") as "open" | "done",
        sourceRef,
...
async function mergeIntoWiki(
  wikiDir: string,
  extraction: WikiExtractionResult,
  sourceRef: string,
  date: string,
  ...
  for (const person of extraction.people) {
    ...
    if (existed) updated++; else written++;
  }
...
async function mergePersonPage(filePath: string, person: WikiPersonEntry, date: string): Promise<void> {
  let content: string;
...
async function mergeProjectPage(filePath: string, project: WikiProjectEntry, date: string): Promise<void> {
  let content: string;
  if (await fileExists(filePath)) {
    content = await fs.readFile(filePath, "utf8");
...
async function mergeTopicPage(filePath: string, topic: WikiTopicEntry, date: string): Promise<void> {
  let content: string;
  if (await fileExists(filePath)) {
    content = await fs.readFile(filePath, "utf8");
...
async function appendDecisions(filePath: string, decisions: WikiDecisionEntry[], date: string, sourceRef: string): Promise<void> {
  let content = "";
  if (await fileExists(filePath)) {
    content = await fs.readFile(filePath, "utf8");
  } else {
  ...
  for (const d of decisions) {
    if (content.includes(sourceRef) && content.includes(d.decision.slice(0, 40))) continue;
...
async function appendActions(filePath: string, actions: WikiActionEntry[], sourceRef: string): Promise<void> {
  let content = "";
  if (await fileExists(filePath)) {
    content = await fs.readFile(filePath, "utf8");
  } else {
  ...
  for (const a of actions) {
    if (content.includes(sourceRef) && content.includes(a.action.slice(0, 40))) continue;
...
export async function regenerateIndex(wikiDir: string): Promise<void> {
  const categories = ["people", "projects", "topics", "decisions", "actions"] as const;
  const lines: string[] = ["# HiDock Wiki", `> Updated: ${new Date().toISOString().slice(0, 10)}`, ""];
...
async function ensureWikiDirs(wikiDir: string): Promise<void> {
  for (const sub of ["people", "projects", "topics", "decisions", "actions"]) {
    await fs.mkdir(path.join(wikiDir, sub), { recursive: true });
  }
}

async function findNotesToCompile(storageDir: string, newSources?: string[]): Promise<string[]> {
...
function extractSummarySection(noteContent: string): string {
  const match = noteContent.match(/## Summary\n([\s\S]*?)(?=\n## Transcript\b|$)/);
  return match?.[1]?.trim() ?? "";
}

function extractNoteMeta(noteContent: string, notePath: string): { date: string; attendee: string; source: string } {
...
function appendToSection(content: string, sectionName: string, entry: string): string {
