...
interface MoonshineJsonOutput {
  segments: {
    text: string;
    speaker_index: number;
    has_speaker_id: boolean;
    start_time: number;
    duration: number;
  }[];
  text: string;
...
export function parseMoonshineOutput(raw: string): {
  text: string;
  speakerSegments?: SpeakerSegment[];
  speakerCount?: number;
} {
  try {
    const json = JSON.parse(raw) as MoonshineJsonOutput;
    if (json.segments && Array.isArray(json.segments)) {
      const segments: SpeakerSegment[] = json.segments.map((s) => ({
        text: s.text,
...
export function formatSpeakerTranscript(segments: SpeakerSegment[]): string {
  ...
  const merged: { speakerIndex: number; startTime: number; texts: string[] }[] = [];
...
export function pickAudioFormat(audioBytes: Uint8Array, fileVersion?: number): {
  extension: "mp3" | "wav";
  mimeType: string;
  detectedFormat: "mp3" | "wav" | "unknown";
} {
