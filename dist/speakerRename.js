 ...
 */
export function buildSpeakerRegex(from, flags = "g") {
 ...
 */
export function renameSpeakerInIndexContent(indexContent, source, from, to) {
    if (!from || !to || from === to)
        return { content: indexContent, replaced: 0 };
    ...
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i] ?? "";
        if (!line.includes(`Source: ${source}`))
            continue;
...
// ---------------------------------------------------------------------------
const WIKI_CATEGORIES = ["people", "projects", "topics", "decisions", "actions"];
/**
 ...
 */
export async function renameSpeakerInWikiDir(wikiDir, from, to) {
    const result = {
        filesUpdated: 0,
        peopleRenamed: 0,
        peopleMerged: 0,
    ...
    for (const category of WIKI_CATEGORIES) {
        ...
        for (const file of entries) {
            ...
            if (category === "people" && skipPeopleFiles.has(file))
                continue;
...
//# sourceMappingURL=speakerRename.js.map