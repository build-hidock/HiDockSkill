...
export function encodeHiDockFrame(commandId, messageId, body = new Uint8Array(0)) {
...
export class HiDockFrameStreamParser {
    ...
    feed(chunk) {
        ...
        while (this.buffer.length >= HIDOCK_FRAME_HEADER_BYTES) {
            if (this.buffer[0] !== HIDOCK_FRAME_HEADER_HI ||
    ...
    findHeaderOffset() {
        for (let index = 0; index < this.buffer.length - 1; index += 1) {
            if (this.buffer[index] === HIDOCK_FRAME_HEADER_HI &&
