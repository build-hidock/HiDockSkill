...
export async function transcribeWithWhisper(input) {
    ...
    const uploadable = await toFile(input.audioBytes, uploadFileName, format.mimeType ? { type: format.mimeType } : undefined);
...
function pickAudioFormat(audioBytes, fileVersion) {
...
function normalizeUploadFileName(sourceFileName, extension) {
